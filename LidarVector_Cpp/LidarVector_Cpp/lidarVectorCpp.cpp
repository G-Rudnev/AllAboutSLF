...
struct Device {
    ...
    Device(PyArrayObject* const pyPntsXY, PyArrayObject* const pyPntsPhi, PyArrayObject* const pyNpnts,
        ...
        pyPntsXY(pyPntsXY), pyPntsPhi(pyPntsPhi), pyNpnts(pyNpnts), pyLinesXY(pyLinesXY), pyNlines(pyNlines), pyGapsIdxs(pyGapsIdxs), pyNGaps(pyNGaps),
        id(id)
    {
        ...
        continuity = safety * sqrt(half_length * half_length + half_width * half_width);

        //связывание с объектами Py
        pNpnts = (long*)PyArray_DATA(pyNpnts);
        pNlines = (long*)PyArray_DATA(pyNlines);
        pNGaps = (long*)PyArray_DATA(pyNGaps);
        ...
        pGapsIdxs = (long*)PyArray_DATA(pyGapsIdxs);
    ...
    bool setParams(PyObject* pyParams) {
        ...
        safety = PyFloat_AsDouble(PyTuple_GetItem(pyParams, 2)) + 1.0;
    ...
    void calcLines() {
        ...
        pLines_x[0] = -0.001 - half_length; //начальный ограничивающий сектор
        pLines_y[0] = 0.0;
        if (pPhi[0] > 0.0) {
            pLines_x[1] = safety * length * cos(pPhi[0] + 2.0 * half_dPhi);
            pLines_y[1] = safety * length * sin(pPhi[0] + 2.0 * half_dPhi);
        }
        else {
            pLines_x[1] = safety * length * cos(pPhi[0] - 2.0 * half_dPhi);
            pLines_y[1] = safety * length * sin(pPhi[0] - 2.0 * half_dPhi);
        ...
        while (fr < *pNpnts)
        {
            ...
            if (!extra)
            {
                ...
                else
                {
                    ...
                    if ((to < *pNpnts) &&
                        (pXY_x[to] != 0.0 || pXY_y[to] != 0.0) &&
                        (sqrt((pXY_x[to] - pXY_x[fr]) * (pXY_x[to] - pXY_x[fr]) + (pXY_y[to] - pXY_y[fr]) * (pXY_y[to] - pXY_y[fr])) <= continuity))
                    {
                        line->setWithTwoPnts(new double[2]{ pXY_x[fr], pXY_y[fr] }, new double[2]{ pXY_x[to], pXY_y[to] });
                        to++;
                        while ((to < *pNpnts) && (pXY_x[to] != 0.0 || pXY_y[to] != 0.0) && (line->getDistanceToPnt(new double[2]{ pXY_x[to], pXY_y[to] }) <= tolerance))
                        {
                            to++;
                            if ((to - fr) % 2 == 0)
                            {
                                long mid_to = fr + (to - fr) / 2;
                    ...
                    if (to - fr > 2)
                    {
                        ...
                        if (edges[0] != 0)
                        {
                            ...
                            if (edges[0] == 1)
                            {
                                if (abs(pXY_x[fr]) > 1e-6)
                                    line->line[0] = pXY_y[fr] / pXY_x[fr];
                                else
                                    line->line[0] = INFINITY;
                                line->line[1] = 0.0;
                                if (!prev_line->isGap)
                                    fr--;
                                to = fr + 1;
            ...
            if (!(line->isGap))
            {
                if (!prev_line->isGap)
                {
                    ...
                    if (((interAngle > pPhi[fr - 1]) || (interAngle < pPhi[fr])) && ((interAngle > pPhi[fr]) || (interAngle < pPhi[fr - 1])))
                    {
                        ...
                        (*pNlines) += 1;
                        if (sqrt((pXY_x[fr] - pXY_x[fr - 1]) * (pXY_x[fr] - pXY_x[fr - 1]) + (pXY_y[fr] - pXY_y[fr - 1]) * (pXY_y[fr] - pXY_y[fr - 1])) > continuity)
                        {
            ...
            else if (fr != 0) // сюда на моих данных не заходит - пока не получилось протестировать, возможно, имеет ошибки
            {
                ...
                else if (sqrt((pXY_x[to] - pXY_x[fr - 1]) * (pXY_x[to] - pXY_x[fr - 1]) + (pXY_y[to] - pXY_y[fr - 1]) * (pXY_y[to] - pXY_y[fr - 1])) > continuity)
                {
                    ...
                    if ((sqrt(pEx_pnt_x * pEx_pnt_x + pEx_pnt_y * pEx_pnt_y) > sqrt(pXY_x[fr - 1] * pXY_x[fr - 1] + pXY_y[fr - 1] * pXY_y[fr - 1])) &&
                        (pEx_pnt_x * pXY_x[fr - 1] > 0.0 or pEx_pnt_y * pXY_y[fr - 1] > 0.0))
                    {
                        pLines_x[*pNlines] = 1e-6;
                        pLines_y[*pNlines] = 1e-6;
                        (*pNlines) += 1;

                        if (sqrt((pXY_x[to] - pEx_pnt_x) * (pXY_x[to] - pEx_pnt_x) + (pXY_y[to] - pEx_pnt_y) * (pXY_y[to] - pEx_pnt_y)) > continuity) {
                    ...
                    else
                    {
                        ...
                        if ((sqrt(pEx_pnt_x * pEx_pnt_x + pEx_pnt_y * pEx_pnt_y) > sqrt(pXY_x[to] * pXY_x[to] + pXY_y[to] * pXY_y[to])) &&
                            (pEx_pnt_x * pXY_x[to] > 0.0 or pEx_pnt_y * pXY_y[to] > 0.0))
                        {
                            if (sqrt((pXY_x[fr - 1] - pEx_pnt_x) * (pXY_x[fr - 1] - pEx_pnt_x) + (pXY_y[fr - 1] - pEx_pnt_y) * (pXY_y[fr - 1] - pEx_pnt_y)) > continuity) {
                                pLines_x[*pNlines] = 0.0;
                                pLines_y[*pNlines] = 0.0;
                                (*pNlines) += 1;
        ...
        if (pPhi[*pNpnts - 1] > 0.0) {
            pLines_x[*pNlines] = safety * length * cos(pPhi[*pNpnts - 1] + 2.0 * half_dPhi);    //конечный ограничивающий сектор
            pLines_y[*pNlines] = safety * length * sin(pPhi[*pNpnts - 1] + 2.0 * half_dPhi);
        }
        else {
            pLines_x[*pNlines] = safety * length * cos(pPhi[*pNpnts - 1] - 2.0 * half_dPhi);    //конечный ограничивающий сектор
            pLines_y[*pNlines] = safety * length * sin(pPhi[*pNpnts - 1] - 2.0 * half_dPhi);
        ...
        (*pNGaps) = 0;
        for (long i = 1; i < (*pNlines); i++) //самая первая точка не смещается
            if (abs(pLines_x[i]) > 1e-5 || abs(pLines_y[i]) > 1e-5)
            {
                pLines_x[i] += mount_x;
                pLines_y[i] += mount_y;
            }
...
PyObject* init(PyObject*, PyObject* o) {
    ...
    if (PyTuple_GET_SIZE(o) == 8) {
        ...
        try {
            devices.push_back(unique_ptr<Device>(new Device(pyPntsXY_, pyPntsPhi_, pyNpnts_, pyLinesXY_, pyNlines_, pyGapsIdxs_, pyNGaps_, pyParams, (long)devices.size())));
